...
inline glm::vec4 unpackR8G8G8A8(uint32_t packed)
{
	return glm::vec4(float((packed >> 24) & 255u), float((packed >> 16) & 255u), float((packed >> 8) & 255u), float(packed & 255u)) / glm::vec4(255.0f);
}
