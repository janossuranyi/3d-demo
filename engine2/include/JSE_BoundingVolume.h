...
class JseBoundingBox;
class JseBoundingSphere
{
public:

	JseBoundingSphere() = default;

	JseBoundingSphere(const vec3& centroid, float radius);
	...
	*/
	float		pointDistance(const vec3& p);
	...
	*/
	bool		rayIntersect(const vec3& p0, const vec3& v, float& t1, float& t2);
	...
	/* Finds an intersection between a line segment and the sphere
	 ...
	 \return true if intersection occured
	*/
	bool		lineIntersect(const vec3& p0, const vec3& p1, float& t1, float& t2);

	/* Decide if this sphere intersect with another
	 \param x sphere to test
	 \return true if intersection occured
	*/
	bool		intersect(const JseBoundingSphere& x) const;

	/* Decide if this sphere intersect with an AABB
	 \param x AABB to test
	 \return true if intersection occured
	*/
	bool		intersect(const JseBoundingBox& x) const;
...
public:
	...
	*/
	JseBoundingSphere	getSphere() const;
	...
	*/
	...
	/* Query the homogeneous positions of the eight corners of the box, in the following order (with \p m=mini, \p M=maxi):
	 ...
	 \return a vector containing the box corners
	 */
	std::vector<vec4> getHomogeneousCorners() const;
	...
	*/
	JseBoundingBox		transformed(const mat4& trans) const;
	...
	*/
	bool			intersect(const vec3& rayOrigin, const vec3& rayDir, vec2& result) const;
	...
	*/
	bool			intersect(const JseBoundingBox& b) const;
...
private:
	vec3 minis = vec3(std::numeric_limits<float>::max());	///< Lower-back-left corner of the box.
	vec3 maxis = vec3(std::numeric_limits<float>::lowest()); ///< Higher-top-right corner of the box.
...
public:
	 ...
	 */
	JseFrustum(const mat4& vp);

	/** Indicate if a bounding box intersect this frustum.
	\param box the bounding box to test
	\return true if the bounding box intersects the frustum.
	*/
	bool	intersects_v1(const JseBoundingBox& box) const;
	bool	intersects_v2(const JseBoundingBox& box) const;
	...
	*/
	...
	bool	intersect(const JseBoundingSphere& sphere) const;
private:

	/** Helper enum for the frustum plane locations. */
	enum JseFrustumPlane {
		LEFT = 0, RIGHT = 1, TOP = 2, BOTTOM = 3, NEAR = 4, FAR = 5, COUNT = 6
	};

	std::array<vec4, JseFrustumPlane::COUNT> _planes; ///< Frustum hyperplane coefficients.
	std::array<vec3, 8> _corners; ///< Frustum corners.

};
#endif // !JSE_BOUNDING_VOLUME_H
