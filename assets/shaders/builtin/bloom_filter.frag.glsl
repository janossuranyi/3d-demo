...
void main() 
{
    vec2 texcoord = texCoord; //screenPosToTexcoord( gl_FragCoord.xy, _fa_freqHigh[0 ] );
    {
        ...
        vec3 c2 = tex2Dlod( samp_tex0, vec4( sampleTexcoord.xy + gBloomParams.zw * vec2( -0.86, -0.5 ), 0, 0 ) ).xyz;
        vec3 c3 = tex2Dlod( samp_tex0, vec4( sampleTexcoord.xy + gBloomParams.zw * vec2( 0.50, -0.86 ), 0, 0 ) ).xyz;
