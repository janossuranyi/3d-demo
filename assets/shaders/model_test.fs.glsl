...
const vec3 kLightColor = vec3(1.0,1.0,1.0) * 10;
...
void main()
{
    ...
    vec3 spceularColor = toLinearRGB(specularGlossiness.rgb);
    ...
    vec3 diffColor = mix(spceularColor, albedo, step(vec3(0.0001), albedo));

    vec3 F0 = spceularColor;
    float roughness = 1-specularGlossiness.a;
    ...
    vec3 kS = F;
    vec3 kD = vec3(1.0) - kS;
    //kD *= 1-max(max(spceularColor.r, spceularColor.g), spceularColor.b);
    ...
    vec3 color = (kD * diffColor / PI + specular) * radiance * NdotL;
