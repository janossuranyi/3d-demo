...
JseBoundingSphere::JseBoundingSphere(const vec3& centroid, float radius)
{
	_center = centroid;
	_radius = radius;
...
bool JseBoundingSphere::contains(const vec3& p)
{
...
float JseBoundingSphere::pointDistance(const vec3& p)
{
...
bool JseBoundingSphere::rayIntersect(const vec3& p0, const vec3& v, float& t1, float& t2)
{
...
bool JseBoundingSphere::lineIntersect(const vec3& p0, const vec3& p1, float& t1, float& t2)
{
	const vec3 v = p1 - p0;
...
bool JseBoundingSphere::intersect(const JseBoundingSphere& x) const
{
...
bool JseBoundingSphere::intersect(const JseBoundingBox& x) const
{
...
vec3 JseBoundingSphere::center() const
{
	return _center;
}

float JseBoundingSphere::radius() const
{
	return _radius;
}

JseBoundingBox::JseBoundingBox(const vec3& v0, const vec3& v1)
{
	minis = glm::min(v0, v1);
	maxis = glm::max(v0, v1);
}

void JseBoundingBox::merge(const JseBoundingBox& box)
{
	minis = glm::min(minis, box.minis);
	maxis = glm::max(maxis, box.maxis);
}

void JseBoundingBox::merge(const vec3& p)
{
	minis = glm::min(minis, p);
	maxis = glm::max(maxis, p);
}

JseBoundingSphere JseBoundingBox::getSphere() const
{
...
vec3 JseBoundingBox::getSize() const
{
	return maxis - minis;
}

std::vector<vec3> JseBoundingBox::getCorners() const {
	return {
...
std::vector<vec4> JseBoundingBox::getHomogeneousCorners() const {
	return {
...
vec3 JseBoundingBox::getCenter() const
{
	return 0.5f * (minis + maxis);
}

JseBoundingBox JseBoundingBox::transformed(const mat4& trans) const {
	JseBoundingBox newBox;
...
bool JseBoundingBox::contains(const vec3& point) const
{
	return all(greaterThanEqual(point, minis)) && all(lessThanEqual(point, maxis));
}

bool JseBoundingBox::empty() const
{
	return minis[0] == std::numeric_limits<float>::max();
}

vec3 JseBoundingBox::getPositiveVertex(const vec3& normal) const
{
...
vec3 JseBoundingBox::getNegativeVertex(const vec3& normal) const
{
...
vec3 JseBoundingBox::min() const
{
	return minis;
}

vec3 JseBoundingBox::max() const
{
	return maxis;
}

bool JseBoundingBox::intersect(const vec3& rayOrigin, const vec3& rayDir, vec2& result) const
{
...
bool JseBoundingBox::intersect(const JseBoundingBox& b) const
{
...
bool JseBoundingBox::intersect(const JseBoundingSphere& x) const
{
...
bool JseFrustum::intersects_v1(const JseBoundingBox& box) const
{
	...
	for (uint pid = 0; pid < JseFrustumPlane::COUNT; ++pid) {
...
bool JseFrustum::intersects_v2(const JseBoundingBox& box) const
{
...
bool JseFrustum::intersect(const JseBoundingSphere& sphere) const
{
	bool result = true;	// TEST_INSIDE;
